resource "aws_instance" "prav-terraform" {
  ...
  provisioner "local-exec" {
    command = "echo ${self.private_ip} > inventory" #this runs during creation
    on_failure = continue #if the provisioner fails it taints the resource and recreats it. To avoid it recreating and continue with resource creation use on_failure = continue.
  }

  provisioner "local-exec" {
    command = "echo Instance is destroyed" #this runs during resource destruction
    when = destroy
